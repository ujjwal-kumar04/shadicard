...
router.post('/signup/verify-otp', async (req, res) => {
  try {
    ...
    // Check if user already exists (in case of resend)
    let user = await User.findOne({ email });
    
    if (user) {
      // Update existing unverified user
      user.name = `${pendingData.firstName} ${pendingData.lastName}`.trim();
      user.password = pendingData.password;
      user.newsletter = pendingData.newsletter;
      ...
      user = new User({
        ...
        newsletter: pendingData.newsletter,
        isVerified: true
      });
      await user.save();
    }
    ...
    res.status(201).json({
      success: true,
      message: 'Account created successfully! Welcome to Shadi Card.',
      token,
      user: {
        id: user._id,
        name: user.name,
